...
def checar_e_alertar_icos(icos):
    """Dispara alertas automáticos para ICOs críticos"""
    for ico in icos:
    ...
    icos_fixos = [
        {
            ...
            "link_info": "https://icodrops.com/idos/",
            "status_manual": "Finalizado"
        },
        {
            ...
            "hora_brt": "11:00 BRT",
            "preco": "$0.91",
            "preco_listing": "$0.85 (pressao venda)",
            "plataformas": "Binance, MEXC, KuCoin",
            "backers": "A confirmar",
            "fdv": "$91M",
            "score": "5/10",
            "unlock": "$23M desbloqueado (2.5% supply)",
            "raised": "N/A",
            "categoria": "DeFi / Infra",
            "descricao": "Unlock massivo de tokens. Alta probabilidade de pressao de venda imediata. Monitorar para short.",
            "risco": "ALTO",
            "roi_alvo": "Negativo (short oportunidade)",
            "link": "https://coinmarketcap.com",
            "link_info": "https://coinmarketcap.com",
        ...
        },
        {
            ...
            "hora_brt": "11:00 BRT",
            "preco": "$0.15 (estimado)",
            "preco_listing": "$0.40 (estimado)",
            "plataformas": "Binance Launchpad, Uniswap",
            "backers": "Binance Labs, a16z crypto",
            "fdv": "$150M",
            "score": "9/10",
            "unlock": "20% no TGE, 80% vesting 12m",
            "raised": "$10M Series A",
            "categoria": "IA / Crypto Intelligence / DeFi",
            ...
            "link_info": "https://icodrops.com",
            "status_manual": "Upcoming"
        },
        {
            "projeto": "Humanity Protocol",
            "token": "RWT",
            "tipo": "TGE / Public Sale",
            "tge_dt": datetime(2026, 3, 18, 16, 0, 0, tzinfo=timezone.utc),
            "tge_str": "18/03/2026",
            "hora_brt": "13:00 BRT",
            "preco": "$0.012",
            "preco_listing": "$0.05 (estimado)",
            ...
            "unlock": "100% no TGE",
            "raised": "$30M total",
            "categoria": "Identidade / Proof of Humanity / Web3",
            ...
            "roi_alvo": "5X-25X",
            "link": "https://gate.io",
            "link_info": "https://humanityprotocol.com",
            "status_manual": "Upcoming"
        },
        {
            "projeto": "Nillion Network",
            "token": "NIL",
            "tipo": "TGE / Airdrop + Sale",
            "tge_dt": datetime(2026, 3, 25, 15, 0, 0, tzinfo=timezone.utc),
            "tge_str": "25/03/2026",
            "hora_brt": "12:00 BRT",
            "preco": "$0.35 (seed)",
            "preco_listing": "$0.80 (estimado)",
            "plataformas": "Binance, Coinbase, Kraken",
            "backers": "a16z, Coinbase Ventures, HashKey",
            "fdv": "$350M",
            "score": "9/10",
            "unlock": "15% TGE, vesting 18m",
            "raised": "$25M",
            "categoria": "Privacy / Compute / AI",
            ...
            "link_info": "https://nillion.com",
            "status_manual": "Upcoming"
        },
    ]

    # Tenta enriquecer com API CoinMarketCal
    try:
        r = requests.get(
        ...
        if r.status_code == 200:
            for ev in r.json().get("body", {}).get("items", [])[:5]:
                try:
                    ...
                    tokens_existentes = [i["token"] for i in icos_fixos]
                    if token_sym not in tokens_existentes:
                        icos_fixos.append({
                            "projeto": ev.get("title","N/A"),
                            "token": token_sym,
                            "tipo": "TGE/ICO",
                            "tge_dt": tge_dt,
                            "tge_str": tge_dt.strftime("%d/%m/%Y"),
                            "hora_brt": "A confirmar",
                            "preco": "A confirmar",
                            "preco_listing": "A confirmar",
                            ...
                            "risco": "MEDIO",
                            "roi_alvo": "A confirmar",
                            "link": ev.get("source","#"),
                            "link_info": ev.get("source","#"),
                            "status_manual": "Upcoming"
                        })
                except:
                    pass
...
def calcular_tempo(tge_dt):
    ...
    if total < 0:
        return delta, "finalizado"
    elif total < 3600:
        return delta, "critico"
    elif total < 86400:
        return delta, "urgente"
    else:
        return delta, "normal"
...
def formatar_tempo(delta):
    ...
    if total < 0:
        ...
        if horas_atras > 0:
            return f"Finalizado ha {horas_atras}h"
...
st.markdown(f"<p style='text-align:center;color:#555;'>Mercado atualizado: {datetime.now().strftime('%d/%m/%Y %H:%M:%S BRT')}</p>", unsafe_allow_html=True)
...
with t5:
    ...
    with st.form("sim_form"):
        c1, c2 = st.columns(2)
        with c1:
            m_sim = st.selectbox("Moeda:", df['name'].tolist())
        with c2:
        ...
        if st.form_submit_button("🚀 SIMULAR"):
            ...
            <div class='metric-card'>
                ...
                <p>ATL: ${d['atl']:.8f} | Data: {d['data_listagem']}</p>
            </div>""", unsafe_allow_html=True)

# ============================================================
# ABA 6: ICO RADAR - PROFISSIONAL COMPLETO
# ============================================================
with t6:
    ...
    with col1:
        data_inicio = st.date_input("De:", value=hoje_utc.date())
    with col2:
        data_fim = st.date_input("Ate:", value=hoje_utc.date() + timedelta(days=30))
    with col3:
    ...
    with col5:
        ...
        if st.button("📨 Alertar Todos Urgentes"):
            for ico in icos:
    ...
    for ico in icos:
        ...
        if tge_dt and not (data_inicio <= tge_dt.date() <= data_fim):
            continue
        if status not in filtro_status:
            continue
        if ico.get("risco","MEDIO") not in filtro_risco:
            continue
        ...
        <div style='{bg_style} padding:25px;border-radius:18px;margin-bottom:20px;'>
            ...
            <!-- BOTOES -->
            <a href='{ico["link"]}' target='_blank' class='link-button'>
                {"ENCERRADO" if status == "finalizado" else "COMPRAR AGORA"}
            </a>
            <a href='{ico["link_info"]}' target='_blank' class='link-button-green'>PESQUISAR PROJETO</a>
            {"<span style='background:#FF000033;color:#FF0000;padding:8px 15px;border-radius:5px;font-size:13px;font-weight:bold;margin-left:5px;border:1px solid #FF0000;'>VENDA ENCERRADA - " + hora_brt + "</span>" if status == "finalizado" else ""}
        </div>
        """, unsafe_allow_html=True)
    ...
    # RODAPE DECISAO
    st.markdown("""
    <div style='background:linear-gradient(135deg,#050505,#0a0500);border:2px solid #FFD700;padding:25px;border-radius:15px;margin-top:10px;'>
        ...
        <div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:20px;'>
            ...
            <div style='border-left:3px solid #FFD700;padding-left:12px;'>
                ...
                <p style='color:#AAA;font-size:12px;margin:5px 0;'>Circle, Ripple, a16z garantem listagens Binance/Coinbase e liquidez imediata.</p>
            </div>
            <div style='border-left:3px solid #00FFFF;padding-left:12px;'>
                ...
                <p style='color:#AAA;font-size:12px;margin:5px 0;'>FDV $39M vs Worldcoin $2B = 50x de room. Quanto menor o FDV, maior o potencial.</p>
            </div>
            <div style='border-left:3px solid #FF8C00;padding-left:12px;'>
                <p style='color:#FF8C00;font-weight:bold;margin:0;'>ROI R$400</p>
                <p style='color:#AAA;font-size:12px;margin:5px 0;'>x $0.20 = R$2.000 (5X) | x $0.39 = R$4.000 (10X) | x $1.00 = R$10.000 (25X)</p>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
